{ config, pkgs, ... }:
let
  qbitPort = toString config.services.qbittorrent.port;
in
...
config.bind(',t', 'hint all spawn curl -X POST -F "urls={hint-url}" -F "sequentialDownload=true" http://localhost:${qbitPort}/api/v2/torrents/add')
config.bind(',P', 'open -p')
config.bind(',v', 'spawn -d mpv {url}')
config.bind(',r', 'restart')
config.bind(';I', 'hint images download')
config.bind('X', 'undo')
config.bind('cf', 'hint links spawn mpv {hint-url}')
config.bind('d', 'scroll-page 0 0.5')
config.bind('gv', 'spawn -d mpv {url}')
